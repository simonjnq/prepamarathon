...
import { addDocumentRowAction } from "./actions";
  ...
  if (!open) {
    return (
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-sm border border-dashed border-line bg-surface-warm px-3 py-1.5 text-xs font-bold text-coral hover:border-coral"
      >
        <Upload size={12} strokeWidth={2.5} /> Téléverser un document
      </button>
    );
  }
  ...
  return (
    <div className="rounded-md border border-line bg-surface-warm p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1">
          ...
          <input
            ...
            placeholder="Ex : Compte rendu test d'effort"
            className="input text-sm"
          />
        </label>
      </div>
      ...
      <label className="mt-3 flex flex-col gap-1">
        <span className="text-xs text-ink-muted">Description (optionnelle)</span>
        <input
          type="text"
          value={description}
